  ...
  def test_sdpa_pattern(self):
    ...
    class M(torch.nn.Module):
      ...
      def forward(self, x, y):
        ...
        q = torch.ops.xla_pattern_marking.mark_tensor(q, "sdpa", pos=0, id=0, is_input=True)
        k = torch.ops.xla_pattern_marking.mark_tensor(k, "sdpa", pos=1, id=0, is_input=True)
        v = torch.ops.xla_pattern_marking.mark_tensor(v, "sdpa", pos=2, id=0, is_input=True)
        attn_out = F.scaled_dot_product_attention(q, k, v, scale=0.25)
        attn_out = torch.ops.xla_pattern_marking.mark_tensor(attn_out, "sdpa", pos=0, id=0, is_input=False, attr={"scale":0.25})
        q, k, v = y.split(128, dim=-2)
        q = torch.ops.xla_pattern_marking.mark_tensor(q, "sdpa", pos=0, id=1, is_input=True)
        k = torch.ops.xla_pattern_marking.mark_tensor(k, "sdpa", pos=1, id=1, is_input=True)
        v = torch.ops.xla_pattern_marking.mark_tensor(v, "sdpa", pos=2, id=1, is_input=True)
        attn_out2 = F.scaled_dot_product_attention(q, k, v, scale=4)
        attn_out2 = torch.ops.xla_pattern_marking.mark_tensor(attn_out2, "sdpa", pos=0, id=0, is_input=False, attr={"scale":2})
    ...
    self.assertTrue('{attributes = {scale = 2.500000e-01 : f32}, name = "sdpa"}}' in stablehlo)
    self.assertTrue('{attributes = {scale = 2 : i64}, name = "sdpa"}}' in stablehlo)
